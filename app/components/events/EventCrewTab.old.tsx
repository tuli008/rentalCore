...
export default function EventCrewTab({ event, crew, roleRequirements: initialRoleRequirements }: EventCrewTabProps) {
  ...
  const handleCrewSelect = async (
    ...
    if (rateType) formData.append("rate_type", rateType);
  ...
  const handleSaveEdit = async (assignmentId: string) => {
    ...
    if (editForm.rate) formData.append("rate", editForm.rate);
    if (editForm.rate_type) formData.append("rate_type", editForm.rate_type);
                        ...
                        <td className="py-3 px-4">
                          <div className="flex gap-2">
                            <select
                              ...
                              onChange={(e) =>
                                setEditForm({ ...editForm, rate_type: e.target.value as any })
                              }
